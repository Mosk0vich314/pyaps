...
Timetrace.acquisition_rate = 50;
Timetrace.runtime = 10;
Timetrace.points_av = Timetrace.scanrate / Timetrace.acquisition_rate;        % points
...
% Plot AI
ax = uiaxes (fig, 'Position',[880 25 550 350]);
ax.XLabel.String = 'Time (s)';
ax.YLabel.String = 'Voltage (V)';
ax.XLim = [0 Timetrace.runtime + 1];
set(ax,'Box','on')
...
%Fun
% Setup UIAxes
u = uiaxes(fig,'Position',[300 300 figPos(3)/3/2 figPos(4)/1.8/2]);
u.XTick = [];
u.YTick = [];
u.Visible = 'off';
% Show Image
...
Layout.ao.Position = [75 175  100 22];
...
for i = 1:Settings.N_output
    ...
    Layout.ao_display(i).Position = [175 + (i-1)*75 175 75 22];
...
Layout.sv.Position = [75 125 100 22];
...
for i = 1:Settings.N_output
    ...
    Layout.rf_display(i).Position = [175 + (i-1)*75 150 75 22];
...
Layout.rf.Position = [75 150 100 22];
...
for i = 1:Settings.N_output
    ...
    Layout.vf(i).Value = 0;
    Layout.vf(i).Position = [175 + (i-1)*75 125 75 22];
end
...
Layout.rr.Position = [75 100 100 22];
Layout.rr.FontWeight = 'bold';

for i= 1:Settings.N_output
    Layout.rr_display(i) = uieditfield(fig,"numeric", 'ValueDisplayFormat','%.3f V/s',...
         "Limits",[0.001 1000],...
        "LowerLimitInclusive","on",...
        "UpperLimitInclusive","on"); 
    Layout.rr_display(i).Value = 1;
    Layout.rr_display(i).Position = [175+(i-1)*75 100 75 22];
end
...
Layout.ai.Position = [75 250 100 22];
...
for i = 1:Settings.N_input
    ...
    Layout.ov(i).Value = 0;
    Layout.ov(i).Position = [175 + (i-1)*75 225 75 22];
end

Data = struct('Layout', Layout, 'Gate', Gate, 'Settings', Settings, 'Timetrace', Timetrace);
...
% reset data arrays in ADwin
for i = 1:Data.Settings.N_input
    SetData_Double(i+1,zeros(Data.Timetrace.acquisition_rate * Data.Timetrace.runtime, 1), 1);
end
...
Data.Timetrace.runtime_counts = Data.Timetrace.acquisition_rate * Data.Timetrace.runtime;
Set_Par(26, Data.Timetrace.runtime_counts);
...
while Process_Status(2)
    for i = 1:Data.Settings.N_input
        ...
        % read all data traces
        timecounter = Get_Par(19);
        tmp = GetData_Double(i+1,1,Data.Timetrace.acquisition_rate * Data.Timetrace.runtime)';

        time = (1:Data.Timetrace.runtime_counts)';

        % reorder data
        idx = mod(timecounter, Data.Timetrace.runtime_counts);
        if idx == 0
            idx = Data.Timetrace.runtime_counts;
        end
        tmp = vertcat(tmp(idx+1:end), tmp(1:idx));
        time = vertcat(time(idx+1:end), time(1:idx));

        % update plots
        clearpoints(hLines(i));
        addpoints(hLines(i), (1:Data.Timetrace.runtime_counts) / Data.Timetrace.acquisition_rate, tmp);
