...
Lockin.dev1.accuracy = 99;
Lockin.dev1.IVgain = 1e8;
...
Lockin.dev1.amplitude_rescaled = Lockin.dev1.amplitude_array * 1e-3 / Lockin.dev1.V_per_V;
Lockin.dev1.ramp_rate_rescaled = Lockin.dev1.ramp_rate * 1e-3 / Lockin.dev1.V_per_V;

%% ramp up drive voltage
Drive.startV = Drive.initV;          % V
Drive.setV = Drive.targetV;          % V
Drive = Apply_fixed_voltage(Settings, Drive);

%% Initialize arrays
N_freq = numel(Lockin.dev1.frequency_array);
N_ampl = numel(Lockin.dev1.amplitude_array);
Timetrace.data_all.X.mean = zeros(N_ampl, N_freq);
Timetrace.data_all.X.std = zeros(N_ampl, N_freq);
Timetrace.data_all.Y.mean = zeros(N_ampl, N_freq);
Timetrace.data_all.Y.std = zeros(N_ampl, N_freq);
Timetrace.data_all.R.mean = zeros(N_ampl, N_freq);
Timetrace.data_all.R.std = zeros(N_ampl, N_freq);
Timetrace.data_all.Theta.mean = zeros(N_ampl, N_freq);
Timetrace.data_all.Theta.std = zeros(N_ampl, N_freq);

%% Init figure
fig = figure('Name','MEMS calibration - frequency dep','Units','normalized');
...
ax1 = gca;
set(gca,'xscale','log')
xlim([Lockin.dev1.frequency_array(1) Lockin.dev1.frequency_array(end)] )
xlabel('Frequency (Hz)');
ylabel('X - component');
...
if N_ampl > 1
    ...
    colormap(colors);
    cbar.Ticks = linspace(min(Lockin.dev1.amplitude_array), max(Lockin.dev1.amplitude_array), N_ampl);
    clim([min(Lockin.dev1.amplitude_array) max(Lockin.dev1.amplitude_array)])
end

%% amplitude dependence
Lockin.dev1.amplitude_rescaled_start = 0;

for index = 1:N_ampl

    %% set lockin amplitude drive voltage
    fprintf('Ramping up AC drive voltage ...')
    ramp_lockin(Lockin.dev1, Lockin.dev1.amplitude_rescaled_start, Lockin.dev1.amplitude_rescaled(index), Lockin.dev1.ramp_rate_rescaled);
    fprintf('done\n')
    ...
    for index2 = 1:N_freq
        ...
        %% set frequency 
        fprintf('%s - Setting frequency = %1.2f Hz...', datetime('now'), Lockin.dev1.frequency_array(index2) )
        Lockin.dev1.dev.set_frequency(Lockin.dev1.frequency_array(index2));
        fprintf('done\n')
        ...
        %% run Timetrace
        fprintf('%s - Running Timetrace : %01d /%01d...', datetime('now'), index2, N_freq)
        ...
        %% make plot
        addpoints(animated_line_X, Lockin.dev1.frequency_array(index2), Timetrace.data_all.X.mean(index, index2));
        addpoints(animated_line_Y, Lockin.dev1.frequency_array(index2), Timetrace.data_all.Y.mean(index, index2));
        addpoints(animated_line_R, Lockin.dev1.frequency_array(index2), Timetrace.data_all.R.mean(index, index2));
        addpoints(animated_line_Theta, Lockin.dev1.frequency_array(index2), Timetrace.data_all.Theta.mean(index, index2));

    end

    %% update amplitude
    Lockin.dev1.amplitude_rescaled_start = Lockin.dev1.amplitude_rescaled(index);
...
Drive.startV = Drive.targetV;          % V
Drive.setV = Drive.endV;          % V
Drive = Apply_fixed_voltage(Settings, Drive);

%% ramp down lockin bias
fprintf('Ramping drive AC voltage ...')
ramp_lockin(Lockin.dev1, Lockin.dev1.amplitude_rescaled_start, 0, Lockin.dev1.ramp_rate_rescaled);
fprintf('done\n')
