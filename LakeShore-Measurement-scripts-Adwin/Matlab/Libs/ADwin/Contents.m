...
% Files
%
%   ADwin_Init           - ADwin_Init  initializes Matlab for communication with ADwin systems.
%   ADwin_Unload         - ADwin_Unload  deletes all ADwin functions from PC memory and releases its
%
...
%   Data2File            - Data2File  saves data of type Long, Float/Float32, or Float64 from a DATA array
%   File2Data            - File2Data  copies data from a file (on the hard disk) into a DATA array of the
%   Data_Length          - Data_Length  returns the length of an ADbasic array of data type LONG or
...
%   GetData_Double       - GetData_Double  transfers parts of a DATA array from an ADwin system into a row
...
%   Get_FPar             - Get_FPar  returns the value of a global variable FPAR.
%   Get_FPar_All         - Get_FPar_All  transfers all global variables FPAR_1...FPAR_80 into a row
%   Get_FPar_Block       - Get_FPar_Block  transfers a specified number of consecutive global variables
%   Get_FPar_Double      - Get_FPar_Double  returns the double value of a global variable FPAR.
%   Get_FPar_All_Double  - Get_FPar_All_Double  transfers all global variables FPAR_1...FPAR_80 into a row
%   Get_FPar_Block_Double - Get_FPar_Block_Double  transfers the specified number of global variables FPAR
%   Get_Last_Error       - Get_Last_Error  returns the number of the error that occurred last in the
%   Get_Last_Error_Text  - Get_Last_Error_Text  returns the error text to a given error number.
%   Get_Par              - Get_Par  returns the value of a global variable PAR.
%   Get_Par_All          - Get_Par_All  transfers all 80 global variables PAR_1... PAR_80 into a row
%   Get_Par_Block        - Get_Par_Block  transfers a specified number of consecutive global variables PAR
...
%   SetData_Double       - SetData_Double  transfers data from a row vector into a DATA array of the ADwin
...
%   Set_FPar             - Set_FPar  sets a global variable FPAR to a specified value.
%   Set_FPar_Double      - Set_FPar_Double  sets a global variable FPAR to a specified value.
%   Set_Language         - Set_Language  sets the language for the error messages.
%   Set_Par              - Set_Par  sets a global variable PAR to the specified value.
