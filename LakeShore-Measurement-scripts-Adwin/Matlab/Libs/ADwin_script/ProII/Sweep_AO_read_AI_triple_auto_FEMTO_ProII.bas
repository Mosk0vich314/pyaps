...
DIM totalcurrent1, totalcurrent2, totalcurrent3 as float
DIM voltage1, voltage2, voltage3 as float
DIM avgcounter,waitcounter,voltagecounter,voltagecounter_new, waitcounterFirstpoint as long
DIM bin1, bin2, bin3 as long
DIM IV1_bit0, IV1_bit1, IV1_bit2, IV1_bit3 as long
DIM IV2_bit0, IV2_bit1, IV2_bit2, IV2_bit3 as long
DIM IV3_bit0, IV3_bit1, IV3_bit2, IV3_bit3 as long
DIM actual_V as long
DIM output_min, output_max, bin_size as float
DIM IV_gain1, IV_gain2, IV_gain3 as float
DIM ADC_gain1, ADC_gain2, ADC_gain3 as long
DIM upper_gain1, upper_gain2, upper_gain3 as float
DIM idx as long
...
INIT:
  ...
  P2_Digout(PAR_7, 28, IV3_bit0)
  P2_Digout(PAR_7, 29, IV3_bit1)
  P2_Digout(PAR_7, 30, IV3_bit2)
  P2_Digout(PAR_7, 31, IV3_bit3)
  ...
  SELECTCASE measureflag 'measurement: 0 = ramp to next voltage point ; 1 = wait ; 2 = measure 
    ...
    CASE 2    
      ...
      IF(avgcounter = PAR_21) THEN
        ...
        FPAR_3 = ((output_min + (voltage3 * bin_size / PAR_21)) * IV_gain3 / 2^ADC_gain3)
        voltage3 = FPAR_3 / IV_gain3
        ...
        ' reverse bias at current limit in first quarter of trace
        IF (((FPAR_9 <> 0) AND (voltagecounter < PAR_23 / 4)) AND (((FPAR_1 > FPAR_9) OR (FPAR_2 > FPAR_9)) OR (FPAR_3 > FPAR_9)) ) THEN
        ...
        ' reverse bias at current limit in second half trace             
        IF (((FPAR_9 <> 0) AND (((FPAR_1 < -1*FPAR_9) OR (FPAR_2 < -1*FPAR_9)) OR (FPAR_3 < -1*FPAR_9)  )) AND ((voltagecounter > PAR_23 / 2) AND (voltagecounter < 3 * PAR_23 / 4))) THEN
        ...
        ' check thresholds ADC 3
        IF (( abs(voltage3) > 8) AND (FPAR_29 > 4)) THEN
          FPAR_29 = FPAR_29 - 1
          measureflag = 3
          voltagecounter = voltagecounter - 1
        ENDIF 
              
        IF (( abs(voltage3) < 0.5) AND (FPAR_29 < upper_gain3)) THEN    
          FPAR_29 = FPAR_29 + 1
          measureflag = 3
          voltagecounter = voltagecounter - 1
        ENDIF
