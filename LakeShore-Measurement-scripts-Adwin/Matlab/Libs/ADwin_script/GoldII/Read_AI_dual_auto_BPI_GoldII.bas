...
INIT:
  ...
  'set IV gain
  IV_gain2 = 10^(-1*FPAR_28)/ 2^ADC_gain2
...
EVENT:
  ...
  IF(avgcounter = PAR_21) THEN
    ...
    totalcurrent1 = (output_min + (totalcurrent1 / PAR_21 * bin_size)) * IV_gain1
    totalcurrent2 = (output_min + (totalcurrent2 / PAR_21 * bin_size)) * IV_gain2
