...
DIM avgcounter,waitcounter,voltagecounter as long
DIM actual_V, dV, maxV_bin, minV_bin, newV as long
DIM bin1 as long
    ...
    CASE 0 'output desired voltage on DAC1
      ...
      SELECTCASE biasdirectionflag
        ...
        CASE 1 
          ...
          IF (waitflag = 0) THEN
            ...
            ELSE
              newV = newV - dV
            ENDIF
            DATA_1[voltagecounter] = newV
            
            waitflag = 1
          
          ENDIF
          
          IF (actual_V <= minV_bin) THEN
            biasdirectionflag = 2       
            repeatflag = 1     
          ENDIF
        ...
        CASE 2 
          ...
          IF (actual_V >= DATA_1[PAR_23]) THEN
            biasdirectionflag = 3
          ENDIF
    ...
    CASE 2    
      ...
      IF(avgcounter = PAR_21) THEN
        ...
        IF ((biasdirectionflag = 0) AND (FPAR_1 > FPAR_5)) THEN ' check for max current for voltage reversal
          biasdirectionflag = 1
          repeatflag = 1
          maxV_bin = DATA_1[voltagecounter]
          minV_bin = 2^16 - maxV_bin
        ENDIF
        
      ENDIF

      ' stop when reached end of vector
      IF ((voltagecounter  = PAR_23 + 1) OR (biasdirectionflag = 3)) THEN   
        END  
      ENDIF
