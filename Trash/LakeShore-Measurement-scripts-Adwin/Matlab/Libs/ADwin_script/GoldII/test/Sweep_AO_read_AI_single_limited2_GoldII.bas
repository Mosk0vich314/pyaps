...
DIM DATA_3[200000] as long     'voltage output  
DIM DATA_2[200000] as float    'ADC1

DIM DATA_6[4] as long         ' new ADC values

DIM measureflag, biasdirectionflag, waitflag, repeatflag as long
DIM totalcurrent1 as float
DIM avgcounter,waitcounter,voltagecounter, voltagecounter2, max_voltagecounter as long
DIM actual_V, dV, newV as long
DIM bin1 as long
...
EVENT:
  ...
  SELECTCASE measureflag 'measurement: 0 = ramp to next voltage point ; 1 = wait ; 2 = measure 
    ...
    CASE 0 'output desired voltage on DAC1
      ...
      SELECTCASE biasdirectionflag
          
        CASE 0 ' unaltered bias voltage

          newV = DATA_1[voltagecounter]
          DATA_3[voltagecounter] = newV
                
        CASE 1  ' ramp down after crossing current limit
          ...
          IF (waitflag = 0) THEN
            ...
            IF (repeatflag = 1) THEN
              repeatflag = 0
            ELSE
              voltagecounter2 = voltagecounter2 - 1
              newV = DATA_1[voltagecounter2]
            ENDIF
            DATA_3[voltagecounter] = newV
          ...
          IF (voltagecounter2 = 2) THEN
            biasdirectionflag = 2   
            voltagecounter2 = 0  
            repeatflag = 1     
          ENDIF
                    
        CASE 2 ' ramp down to Vmin 
          
          IF (waitflag = 0) THEN
          
            voltagecounter2 = voltagecounter2 + 1
            newV = 2^16 - DATA_1[voltagecounter2]
          
            DATA_3[voltagecounter] = newV
            waitflag = 1
            
            IF (voltagecounter2 = max_voltagecounter) THEN
              biasdirectionflag = 3          
            ENDIF
            
          ENDIF
          
        CASE 3 ' ramp up to zero 
      ...
      IF(newV > actual_V) THEN INC(actual_V)      
      IF(newV < actual_V) THEN DEC(actual_V) 
      IF  (actual_V = newV) THEN 
        measureflag = 1 
      ENDIF
    ...
    CASE 2    
      ...
      totalcurrent1 = totalcurrent1 + ((output_min + (bin1 * bin_size / 2^PAR_1)) * IV_gain1)
      ...
      IF(avgcounter = PAR_21) THEN
        ...
        IF ((biasdirectionflag = 0) AND (FPAR_1 > FPAR_5)) THEN ' check for max current for voltage reversal
          biasdirectionflag = 1
          repeatflag = 0
          max_voltagecounter = voltagecounter
          voltagecounter2 = voltagecounter
        ENDIF
        
      ENDIF

      ' stop when reached end of vector
      IF ((voltagecounter  = PAR_23 + 1) OR (biasdirectionflag = 4)) THEN   
        END  
      ENDIF
