classdef Temperature_controller_Lakeshore336 < handle
    ...
    methods
        ...
        function T_controller = Temperature_controller_Lakeshore336(address)

            address = upper(address);
            T_controller.serial_address = address;

            if regexp(address,'GPIB')
                    T_controller.device = visa('ni',address);

            elseif regexp(address,'COM')
                    T_controller.device = serial(address,'BaudRate',57600,'StopBits',1,'Parity','odd','DataBits',7);
            else
                    disp('Unknown connection protocol')
            end
        ...
        function [time,error] = wait_T_setpoint(varargin)
            ...
            T_setpoint = str2double(fscanf(T_controller.device));
