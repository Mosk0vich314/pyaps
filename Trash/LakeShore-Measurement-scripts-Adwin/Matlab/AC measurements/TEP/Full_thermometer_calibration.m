% This script measures the resistance of the heater, thermometer 1, thermometer 2 for various bath temperatures,
% and heater currents. For each measurement, and long measurement at low sampling rate is performed, followed
% by a short measurement at high sample rate that is used for averaging.
...
%% Settings
Settings.save_dir = 'E:\Samples\FrevaPaal_graphene_TEP\FullCalibrationAndFreq_NoGraphene';
Settings.sample = 'NoGraphene';
Settings.ADC_gain = [0 0 0 0 0 0 0 0]; % 2^N
Settings.get_sample_T = 'Lakeshore325'; % {'', 'Lakeshore336', 'Lakeshore325', 'Oxford_ITC'}
Settings.temperature_no_heater = [300 200 100 50];           % bath temperature 5:2.5:301
Settings.temperature_heater = [300 200 100 50];           % bath temperature [7.5 25 50 75 100 125 150 175 200 225 250 275 300]
Settings.conductance_frequency = 33;

% Settings.frequency_array = 10:5:20; 
Settings.frequency_array = 10.^linspace(1, log10(2738), 27);           % Hz
Settings.frequency_array(5) = 21.7;
Settings.frequency_array(6) = 29;
...
Settings.AC_conductance_heater_dep = 0;            % use MD option for measuring AC the codnuctance during heater dependence
...
[Timetrace.no_heater.long, Settings] = Define_arrays_T_calibration(Settings, Timetrace, [Settings.N_temp_no_heater 1]);
[Timetrace.no_heater.short, Settings] = Define_arrays_T_calibration(Settings, Timetrace, [Settings.N_temp_no_heater 1]);
[Timetrace.heater.long, Settings] = Define_arrays_T_calibration(Settings, Timetrace, [Settings.N_temp_no_heater Settings.N_amplitude_heater_current Settings.N_freq]);
[Timetrace.heater.short, Settings] = Define_arrays_T_calibration(Settings, Timetrace, [Settings.N_temp_no_heater Settings.N_amplitude_heater_current Settings.N_freq]);
...
for i = 1 : Settings.N_temp_no_heater
    ...
    %% set conductance frequency
    fprintf('%s - Heater dependence - Setting AC heater frequency to %1.3f Hz...', datetime('now'), Settings.conductance_frequency)
    for ii = 1 : Timetrace.N_devices
    ...
    %% Init long measurements with heater off
    Timetrace.T = zeros(1, Timetrace.get_T);
    Timetrace.T_time = zeros(1, 1);

    %% set data rate
    Timetrace.datarate = Lockin.dev1.datarate_no_heater;
    for ii = 1 : Timetrace.N_devices
    ...
    Lockin.dev1.datarate_no_heater_new = Timetrace.datarate;
    ...
    if ismember(Settings.temperature_no_heater(i), Settings.temperature_heater)
        ...
        %% ramp up AC for conductance on thermometer 1
        if Settings.AC_conductance_heater_dep
            Lockin.dev2.dev.set_output_channel(0, 1); % switch off channel 1
            Lockin.dev2.dev.set_output_channel(1, 4); % switch on channel 4

            fprintf('%s - Setting AC current on T1 to %1.2f uA...', datetime('now'), Lockin.dev2.amplitude_Ibias * 1e6)
            for ii = 2
                ramp_lockin(Lockin.(Lockin.device_names{ii}), 0, Lockin.(Lockin.device_names{ii}).amplitude_Ibias_rescaled, Lockin.(Lockin.device_names{ii}).ramp_rate_rescaled, 4);
            end
            fprintf(' done \n')
        end
        ...
        for f = 1:Settings.N_freq

%% 
            fprintf('%s - Setting frequency heater to %1.2f Hz...\n', datetime('now'), Settings.frequency_array(f))

            %% set frequency for heater measurement
            for ii = 1 : Timetrace.N_devices
                Lockin.(Lockin.device_names{ii}).dev.set_frequency(Settings.frequency_array(f));
            end
            ...
            %% update timeconstant
            % Lockin.dev1.timeconstant = 10 / min(Settings.frequency_array(f)*2, Settings.conductance_frequency);
            % Timetrace.wait_time = Get_lockin_waiting_period(Lockin.dev1.filter_order, Lockin.dev1.accuracy) * Lockin.dev1.timeconstant;      % s
            % 
            ...
            for j = 1 : Settings.N_amplitude_heater_current
                ...
                if Settings.AC_conductance_heater_dep
                    gains = {Lockin.dev1.DC_current * Lockin.dev1.Vgain, ...
                        [ Lockin.dev2.DC_current * Lockin.dev2.Vgain * ones(3,1); Lockin.dev2.amplitude_Ibias * Lockin.dev2.Vgain; ],...
                        [ Lockin.dev2.DC_current * Lockin.dev2.Vgain * ones(3,1); Lockin.dev2.amplitude_Ibias * Lockin.dev2.Vgain; ]};
                else
                ...
                %% make plot heater dependence conductance AC
                if Settings.AC_conductance_heater_dep
                    fig_heater_dep_cond = Realtime_timetrace_T_calibration_heater_dep_conductance(Settings, Timetrace, Lockin, figure_heater_dep_conductance);
                end
            end

            %% ramp down AC conductance
            if Settings.AC_conductance_heater_dep
                fprintf('%s - Setting AC current on T1 to %1.2f uA...', datetime('now'), 0)
                for ii = 2
                    ramp_lockin(Lockin.(Lockin.device_names{ii}), Lockin.(Lockin.device_names{ii}).amplitude_Ibias_rescaled, 0, Lockin.(Lockin.device_names{ii}).ramp_rate_rescaled, 4);
                end
                Lockin.dev2.dev.set_output_channel(0, 4); % switch on channel 4
                Lockin.dev2.dev.set_output_channel(1, 1); % switch off channel 1
        ...
        fprintf('%s - Conductance measurement - Setting AC amplitude to %1.3fmA...', datetime('now'), Lockin.dev2.amplitude_Ibias * 1e3)
        for ii = 1 : Timetrace.N_devices
            ramp_lockin(Lockin.(Lockin.device_names{ii}), 0, Lockin.(Lockin.device_names{ii}).amplitude_Ibias_rescaled, Lockin.(Lockin.device_names{ii}).ramp_rate_rescaled);
        end
        fprintf(' done\n')
